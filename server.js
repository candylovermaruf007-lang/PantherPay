...
// Robust DB module resolution to handle different deploy root directories (root vs src)
function resolveDbModule() {
  const candidates = [
    path.join(__dirname, 'src', 'db', 'db.js'),
    path.join(__dirname, 'db', 'db.js'),
  ];
  for (const p of candidates) {
    if (fs.existsSync(p)) return p;
  }
  throw new Error('DB module not found. Tried: ' + candidates.join(' | '));
}
const { initDb, getSettings, runAsync } = require(resolveDbModule());
